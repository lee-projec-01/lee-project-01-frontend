...
function ChatRoomDetail() {
    ...
    // // STOMP
    // const socket = new SockJs("http://localhost:8080/api/chats");
    ...
    //         client.subscribe("/api/chats/sub" , (msg) => {
  ...
  useEffect(() => {
    ...
    const client = new Client({
        ...
        onConnect : () => {
            console.log("STOMP Connected");
            if(chatRoom.id){
            // client.subscribe(`ws://localhost:8080/api/sub/${chatRoom.id}` , (msg) => {
            //         const body = msg.body();
            //         setMessage(JSON.parse(body));
            // })
            client.subscribe(`wss://day6.duckdns.org/api/v1/sub/test`, (msg) => {
              const body = msg.body();
              setMessage(JSON.parse(body));
      })
            }
